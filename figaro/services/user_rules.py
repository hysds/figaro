...
def add_status_mappings(es_url, es_index):
    ...
    # get all mappings from figaro status indexes using alias
...
def process_this():
    ...
    #param_temp = [{"name":"submitter","from":"value","value":"figaro"},{"name":"submitter_rule","from":"value","value":json.dumps(rule)}]

    #hysds_commons.job_rest_utils.single_process_and_submission(app.config["ES_URL"], {}, rule_temp , hysdsio={"id":"internal-temporary-wiring","params":param_temp,"job-specification":app.config["JOB_SUBMISSION_JOB_SPEC"]})
